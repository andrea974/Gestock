...
(12,	'Andréa',	'andrea.bigot974@gmail.com',	262,	'compteVole',	'gg');
...
CREATE TABLE `vue_vet_disponibilite` (`idVet` int(11), `listeNumCouleurDispo` varchar(256), `listeTailleDispo` text);


DROP TABLE IF EXISTS `vue_vet_disponibilite`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_vet_disponibilite` AS select `v`.`id` AS `idVet`,(select group_concat(`vcl2`.`num` separator ',') from `vet_couleur` `vcl2` where ((`vcl2`.`idVet` = `v`.`id`) and (`vcl2`.`dispo` = 1))) AS `listeNumCouleurDispo`,group_concat(distinct `vt`.`taille` separator ',') AS `listeTailleDispo` from ((`vetement` `v` left join `vet_couleur` `vcl` on((`vcl`.`idVet` = `v`.`id`))) left join `vet_taille` `vt` on((`vt`.`idVet` = `v`.`id`))) group by `v`.`id`;
